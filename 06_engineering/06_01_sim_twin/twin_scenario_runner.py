...
from constants import (
    INVALID_MODE_REQUEST_REASON,
    TOPIC_EVENT_ALARM,
    TOPIC_EVENT_AUDIT,
    TOPIC_EVENT_FAULT,
    TOPIC_HEALTH_HEARTBEAT,
    TOPIC_STATE_STATUS,
    TOPIC_STATE_TELEMETRY,
)
...
def execute_startup_scenario() -> ScenarioResult:
    ...
    passed = runtime.state.value == "IDLE" and _has_topics(
        runtime,
        [TOPIC_EVENT_AUDIT, TOPIC_STATE_STATUS, TOPIC_HEALTH_HEARTBEAT],
    )
...
def execute_manual_mode_scenario() -> ScenarioResult:
    ...
    runtime.handle(manual_command(runtime.config.demo_manual_linear, runtime.config.demo_manual_angular))
    actual_transition = " | ".join(f"{prev} -> {curr}" for _, prev, curr, _ in runtime.transition_log)
    passed = runtime.state.value == "MANUAL" and _has_topics(
        runtime,
        [TOPIC_EVENT_AUDIT, TOPIC_STATE_STATUS, TOPIC_STATE_TELEMETRY, TOPIC_HEALTH_HEARTBEAT],
    )
    return ScenarioResult(
        name="manual mode",
...
def execute_obstacle_scenario() -> ScenarioResult:
    ...
    passed = runtime.state.value == "SAFE_STOP" and _has_topics(
        runtime,
        [TOPIC_EVENT_AUDIT, TOPIC_STATE_STATUS, TOPIC_EVENT_ALARM, TOPIC_HEALTH_HEARTBEAT],
    )
    return ScenarioResult(
        name="obstacle",
...
def execute_estop_scenario() -> ScenarioResult:
    ...
    passed = runtime.state.value == "IDLE" and _has_topics(
        runtime,
        [TOPIC_EVENT_AUDIT, TOPIC_STATE_STATUS, TOPIC_EVENT_ALARM, TOPIC_EVENT_FAULT, TOPIC_HEALTH_HEARTBEAT],
    )
    return ScenarioResult(
        name="estop",
...
def execute_invalid_command_scenario() -> ScenarioResult:
    runtime = TwinRuntime()
    runtime.handle(startup_ok())
    runtime.handle(invalid_command(INVALID_MODE_REQUEST_REASON))
    actual_transition = " | ".join(f"{prev} -> {curr}" for _, prev, curr, _ in runtime.transition_log)
    passed = runtime.state.value == "IDLE" and _has_topics(runtime, [TOPIC_EVENT_AUDIT])
    return ScenarioResult(
